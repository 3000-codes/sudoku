  ...
  function _isCellRelatedToSelectedCell(row: number, column: number) {
    ...
    return [[0, 3, 0, 3],
      ...
      if (rowOfSelectedCell > array[0] - 1 && row > array[0] - 1 &&
                  ...
                  columnOfSelectedCell < array[3] && column < array[3]) { return true }
      return false
    })
  }
  ...
  function _selectedCell (indexOfArray: number, value: string, highlight: string) {
    if (value !== '0') {
      if (initArray[indexOfArray] === '0') {
        return (
          <td className={`game__cell game__cell--userfilled game__cell--${highlight}selected text-center`} key={indexOfArray} onClick={() => { props.onClick(indexOfArray) }}>{value}</td>
        )
      } else {
        return (
                ...
                <tr className="game__row" key={row}>
                  {
                    rows.map((column) => {
                      ...
                      if (fastMode) {
                        if (numberSelected !== '0' && _isCellSameAsSelectedCell(row, column) === true) {
                          return _selectedCell(indexOfArray, value, '')
                        } else {
                          return _unselectedCell(indexOfArray, value)
                        }
                      } else {
                        if (cellSelected !== -1 && _isCellSameAsSelectedCell(row, column) === true) {
                          return _selectedCell(indexOfArray, value, '')
                        } else {
                          return _unselectedCell(indexOfArray, value)
                        }
